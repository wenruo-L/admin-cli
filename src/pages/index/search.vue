<template>
  <div class="block-searchs" @click.self="handleEsc">
    <div class="block-searchs__title">菜单搜索</div>
    <div class="block-searchs__content">
      <div class="block-searchs__form">
        <el-input
          placeholder="请输入搜索内容"
          v-model="value"
          :autofocus="true"
        ...
        <p>
          <el-tag>你可以使用快捷键esc或点击空白区域关闭</el-tag>
        </p>
      </div>
      <div class="block-searchs__list">
        <el-scrollbar class="block-searchs__scrollbar">
          <div
            class="block-searchs__item"
            v-for="(item, index) in menus"
            :key="index"
            @click="handleSelect(item)"
          >
            <span class="block-searchs__item-title">{{ item[labelKey] }}</span>
            <div class="block-searchs__item-path">
              {{ item[pathKey] }}
            </div>
          </div>
        </el-scrollbar>
